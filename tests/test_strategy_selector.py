...
class TestBoundaries:
    def test_exactly_low_threshold(self, selector: StrategySelector) -> None:
        # risk_score == low_threshold → NOT < threshold → should be rag
        low = selector.config.get("strategy", {}).get("thresholds", {}).get("low", 0.35)
        result = selector.select({"risk_score": low, "hallucination_type": "none"})
        assert result == "rag"
